...
- `pipeline/` — content extraction + build scripts
  - `content/*.json` — structured content (7 gifts, 35 archetypes, 77 questions + scoring maps, distress model, interaction model)
    ...
    - `archetype-pressure.json` — **retained but not rendered.** All 35 archetypes under pressure (composite pattern name, three parallel descents, earliest tell, synchronisation pattern, missing brake). Re-enable by restoring the `archetypePressureSection(a)` call in `archetypePage()` and the `pressure:` field in `dataJs()`.
...
The archetype pages currently carry only their original canon sections; the archetype pressure layer is authored and kept in `pipeline/content/archetype-pressure.json` but is not rendered.
